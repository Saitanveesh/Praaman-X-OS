...
export interface MAVLinkReadonlyStatus { connected: boolean; read_only: boolean; commands_enabled: boolean; host: string; port: number; protocol: string; endpoint: string; last_error?: string | null; last_message_time?: string | null; message_counts: Record<string, number>; received_since_connect: boolean; supported_protocols: Record<string, string>; ok?: boolean; }
export interface SITLReadiness { status: string; read_only: boolean; command_sending_enabled: boolean; expected_host: string; expected_port: number; telemetry_source: string; mavlink_provider_status: string; mission_planner_role: string; pramaanx_role: string; checklist: string[]; warnings: string[]; recommended_next_step: string; sitl_supported: boolean; mavlink_readonly_supported: boolean; recommended_sitl_out: string; mission_planner_coexistence: boolean; documentation_only_commands: string[]; }
export interface SystemStatus { app_name: string; company: string; app_stage: string; simulation_only: boolean; hardware_commands_enabled: boolean; mavlink_command_sending_enabled: boolean; pufshield_integrated: boolean; active_telemetry_source: string; backend_status: string; warning_count: number; }
export interface TelemetryIntelligence { drone_id: string; battery_level: number; battery_risk: string; altitude_status: string; speed_status: string; gps_status: string; telemetry_freshness: string; recommended_action: string; warnings: string[]; summary: string; timestamp: string; }
export interface LinkIntelligence { drone_id: string; link_state: string; link_quality: string; operator_message: string; risk_level: string; recommended_action: string; timestamp: string; }
export interface IntelligenceSummary { drone_id: string; telemetry: TelemetryIntelligence; link: LinkIntelligence; warnings: string[]; simulation_only: boolean; hardware_commands_enabled: boolean; }
export interface MissionExport { format: string; mission: MissionDraft; waypoints: MapWaypoint[]; validation: MissionValidation; exported_at: string; hardware_upload_enabled: boolean; }
export interface MissionImportPayload { format: string; mission: MissionDraftCreate & { mission_id?: string }; waypoints: MapWaypointCreate[]; }
export interface MissionReport { mission_id: string; name: string; drone_id: string; vehicle_type: string; status: string; waypoint_count: number; estimated_distance_m: number; estimated_duration_s: number; max_altitude_m: number; min_altitude_m: number; lost_link_action: string; validation_errors: string[]; validation_warnings: string[]; simulation_only: boolean; hardware_upload_enabled: boolean; }
