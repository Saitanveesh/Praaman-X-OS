...
from app.schemas.mission import MapWaypointCreate, MissionDraftCreate, MissionValidationRead
...
class MissionDraftService:
    ...
    def validate_mission(self, db: Session, mission_id: str) -> MissionValidationRead | None:
        ...
        if mission.vehicle_type == VehicleType.FIXED_WING.value and not any(wp.action in {WaypointAction.LOITER.value, WaypointAction.RETURN_POINT.value} for wp in waypoints):
            warnings.append("Fixed-wing mission drafts should include loiter or return-point behavior for resilience.")
        if mission.vehicle_type == VehicleType.QUADCOPTER.value:
            warnings.append("Quadcopter mission drafts can use hover/hold behavior through governance-approved simulation modes.")
        mission.status = MissionDraftStatus.INVALID.value if errors else MissionDraftStatus.VALIDATED.value
        db.commit()
        return MissionValidationRead(mission_id=mission_id, status=mission.status, valid=not errors, errors=errors, warnings=warnings)
